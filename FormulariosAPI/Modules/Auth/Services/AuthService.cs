using ErrorOr;
using FormulariosAPI.Constants;
...
namespace FormulariosAPI.Modules.Auth.Services;
...
public class AuthService : IAuthService
{
    ...
    public async Task<ErrorOr<AuthResponse>> Login(AuthRequest request)
    {
        var user = await _userManager.FindByEmailAsync(request.Email);

        if (user == null)
        {
            return Error.Validation("Usario no encontrado", $"El usuario con email {request.Email} no existe");
        }
        ...
        {
            return Error.Validation("Credenciales", "Las credenciales son incorrectas");
        }
    ...
    public async Task<ErrorOr<RegistrationResponse>> Register(RegistrationRequest request)
    {
        ...
        {
            return Error.Validation("UserName", "El username ya fue tomado por otra cuenta");
        }

        var existingEmail = await _userManager.FindByEmailAsync(request.Email);
        if (existingEmail != null)
        {
            return Error.Validation("Email", "El email ya fue tomado por otra cuenta");
        }

        var rol = await _roleManager.FindByNameAsync(request.Rol);
        if (rol == null)
        {
            return Error.Validation("Rol", $"El rol {request.Rol} no existe");
